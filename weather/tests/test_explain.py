...
class TestExplainFlagImpliesDryRun(unittest.IsolatedAsyncioTestCase):

    @patch("weather.strategy.get_noaa_forecast", new_callable=AsyncMock)
    async def test_explain_forces_dry_run(self, mock_noaa):
        ...
        bridge.execute_maker_order = AsyncMock()
...
class TestExplainStatsAccumulation(unittest.IsolatedAsyncioTestCase):

    @patch("weather.strategy.get_noaa_forecast", new_callable=AsyncMock)
    async def test_explain_stats_logged(self, mock_noaa):
...
class TestExplainFilterReasonsTracking(unittest.IsolatedAsyncioTestCase):

    @patch("weather.strategy.get_noaa_forecast", new_callable=AsyncMock)
    async def test_filter_reasons_tracked(self, mock_noaa):
...
class TestExplainSummaryFormat(unittest.IsolatedAsyncioTestCase):

    @patch("weather.strategy.get_noaa_forecast", new_callable=AsyncMock)
    async def test_summary_has_separator_lines(self, mock_noaa):
